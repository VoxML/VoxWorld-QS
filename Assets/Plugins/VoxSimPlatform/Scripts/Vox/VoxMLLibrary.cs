...
namespace VoxSimPlatform {
    namespace Vox {
        ...
        public class VoxMLLibrary : MonoBehaviour {
            ...
            public Dictionary<string, string> VoxMLPredicateDict;
            ...
            public class DebugPreview : Editor {
                public override void OnInspectorGUI() {
                    ...
                    // some styling for the header, this is optional
                    GUILayout.Label("VoxML Predicates", bold);
                    ...
                    if (((VoxMLLibrary)target).VoxMLEntityTypeDict != null)
                    {
                        foreach (string item in ((VoxMLLibrary)target).VoxMLPredicateDict.Keys)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label(item, GUILayout.Width(150));
                            GUILayout.Label(((VoxMLLibrary)target).VoxMLPredicateDict[item]);
                            GUILayout.EndHorizontal();
                        }
                    }
